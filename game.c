        ...
        for (int x = -1; x < 9; x++) {
            ...
            if (y != -1 && y != 8) {
                if (x == -1 || x == 8) {
                    ...
                    if (P != NULL) {
                        char X = get_piece_letter(P->unittype);
                        if (P->color == WHITE) {
                            printf(ANSI_WHITE "%c" ANSI_RESET, X);
                        } else {
                            printf(ANSI_RED "%c" ANSI_RESET, X);
                        };
                    } else {
...
char get_piece_letter(UnitType unit) {
    switch (unit) {
    case KING:
        return 'K';
    case QUEEN:
